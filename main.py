...
def one_year_check(stockTicker):
    ...
    if ((pd.Timestamp("now") - pd.to_datetime(list_date)) < pd.Timedelta("1 Y")):
        return True
    fiveyear = r.get_historicals(stockTicker,span='year',bounds='regular')
    closingPrices = []
...
def golden_cross(stockTicker, n1, n2, days, direction=""):
    ...
    Returns:
        ...
        0 if there is no cross between the indicators
        -1 if the short-term indicator crosses below the long-term one
        False if direction == "above" and one_year_check(stockTicker) returns False, meaning that we're considering whether to
            buy the stock but it hasn't risen overall in the last five years, suggesting it contains fundamental issues
    """
    if(direction == "above" and not one_year_check(stockTicker)):
        return False
    ...
    series = [price.rename("Price"),dates.rename("Dates")]
...
def ema_today(stockTicker):
    history = r.get_historicals(stockTicker,span='month',bounds='regular')
    ...
    ##12
    for item in history:
        if item['begins_at'] > str(today - timedelta(days=12)):
            closingPrices12.append(float(item['close_price']))
        else:
            pass
    sma12 = sum(closingPrices12) / len(closingPrices12)
    weight12 = 2 / (len(closingPrices12) + 1)
    ema12 = float((pricenow - sma12) * weight12 + sma12)
    ##26
    for item in history:
        if item['begins_at'] > str(today - timedelta(days=26)):
            closingPrices26.append(float(item['close_price']))
        else:
            pass
...
def ema_yesterday(stockTicker):
    ...
    ##12
    for item in history:
        if item['begins_at'] > str(yesterday - timedelta(days=12)):
            closingPrices12.append(float(item['close_price']))
        else:
            pass
    sma12 = sum(closingPrices12) / len(closingPrices12)
    ...
    ##26
    for item in history:
        if item['begins_at'] > str(yesterday - timedelta(days=26)):
            closingPrices26.append(float(item['close_price']))
        else:
            pass
    sma26 = sum(closingPrices26) / len(closingPrices26)
